<?php
/**
 * Edit Organization Card Type
 * 
 * Form for editing existing card specifications.
 */
...
if (empty($cardId)) {
    header('Location: /public/orgs/cards_list.php');
    exit;
}

$card = r_load_org_card($orgId, $cardId);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    org_cards_update($orgId, $cardId, $cardData);
    
    header('Location: /public/orgs/cards_list.php?updated=1');
    exit;
}

$loanPolicy = $card['loan_policy'] ?? [];
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Edit Card Type - R-Card</title>
        ...
        <main class="flex-1 p-8">
            <div class="max-w-4xl mx-auto">
                ...
                <div class="mb-8">
                    ...
                    <h1 class="text-4xl font-bold mb-2">Edit Card Type</h1>
                    <p class="text-gray-400">Update card specification</p>
                </div>
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="text" name="card_name" class="input-field" required
                                       value="<?php echo htmlspecialchars($card['card_name'] ?? ''); ?>">
                            </div>

                            <div>
                                <label class="block text-sm font-medium mb-2">Card Type *</label>
                                <select name="card_type" id="card_type" class="select-field" required>
                                    <option value="debit" <?php echo ($card['card_type'] ?? '') === 'debit' ? 'selected' : ''; ?>>Debit</option>
                                    <option value="credit" <?php echo ($card['card_type'] ?? '') === 'credit' ? 'selected' : ''; ?>>Credit</option>
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" name="public_identifier" class="input-field" required
                                       value="<?php echo htmlspecialchars($card['public_identifier'] ?? ''); ?>">
                                <p class="text-sm text-gray-400 mt-1">Used in API calls to identify this card type</p>
                            </div>

                            <div>
                                <label class="block text-sm font-medium mb-2">Description</label>
                                <textarea name="description" class="input-field" rows="3"><?php echo htmlspecialchars($card['description'] ?? ''); ?></textarea>
                            </div>
                        </div>
                    </div>

                    <!-- Credit Card Settings -->
                    <div id="credit-settings" class="glass-card p-6 <?php echo ($card['card_type'] ?? '') !== 'credit' ? 'hidden' : ''; ?>">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="number" name="credit_limit" class="input-field" 
                                       min="0" step="100" value="<?php echo $card['credit_limit'] ?? 5000; ?>">
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" name="interest_rate_monthly" class="input-field" 
                                       min="0" max="100" step="0.1" value="<?php echo $card['interest_rate_monthly'] ?? 1.5; ?>">
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="space-y-4">
                            <div class="flex items-center gap-3">
                                <input type="checkbox" name="loan_enabled" id="loan_enabled" 
                                       class="w-5 h-5" <?php echo ($loanPolicy['loan_enabled'] ?? false) ? 'checked' : ''; ?>>
                                <label for="loan_enabled" class="font-medium">Enable Loans</label>
                            </div>

                            <div id="loan-settings" class="space-y-4 <?php echo !($loanPolicy['loan_enabled'] ?? false) ? 'hidden' : ''; ?>">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input type="number" name="loan_max_amount" class="input-field" 
                                               min="0" step="100" value="<?php echo $loanPolicy['loan_max_amount'] ?? 2000; ?>">
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <input type="number" name="loan_max_year" class="input-field" 
                                               min="0" step="100" value="<?php echo $loanPolicy['loan_max_year'] ?? 5000; ?>">
                                    </div>

                                    <div>
                                        <label class="block text-sm font-medium mb-2">Monthly Interest Rate (%)</label>
                                        <input type="number" name="loan_interest_rate_monthly" class="input-field" 
                                               min="0" max="100" step="0.1" value="<?php echo $loanPolicy['loan_interest_rate_monthly'] ?? 2.0; ?>">
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <input type="number" name="loan_min_wait_days" class="input-field" 
                                               min="1" max="365" value="<?php echo $loanPolicy['loan_min_wait_days'] ?? 7; ?>">
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <input type="number" name="loan_max_days" class="input-field" 
                                               min="1" max="180" value="<?php echo $loanPolicy['loan_max_days'] ?? 90; ?>">
                                    </div>
                                </div>
                            </div>
                        </div>
